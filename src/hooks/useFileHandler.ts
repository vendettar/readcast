...
export function useFileHandler() {
    ...
    const createOrUpdateSession = useCallback(async () => {
        ...
        try {
            ...
            log('[FileHandler] Created session:', sessionId);
        } catch (err) {
            logError('[FileHandler] Failed to create session:', err);
        }
    }, [setSessionId]);
    ...
    const processFiles = useCallback(async (files: FileList | File[]) => {
        ...
        for (const file of Array.from(files)) {
            if (file.name.endsWith('.mp3') || file.name.endsWith('.m4a') ||
                ...
                loadAudio(file);
                // Save to DB for LocalFiles visibility
                try {
                    const audioId = await DB.addAudio(file, file.name);
