...
import { CLICK_DELAY_MS, findWordAtPoint, isLookupEligible, loadDictCache } from '../../libs/selection';
...
export function useSelectionEvents(
    ...
    // Store latest actions in ref to avoid handler re-creation
            ...
            if (wordInfo && isLookupEligible(wordInfo.word)) {
                ...
                clickTimerRef.current = setTimeout(() => {
                    ...
                    if (sel && sel.isCollapsed) {
                        actionsRef.current.lookupWord(wordInfo.word, rect.right + 10, rect.top + rect.height / 2);
                    }
                }, CLICK_DELAY_MS);
            }
        }
    ...
    }, [containerRef, handleMouseDown, handleMouseUp, handleMouseMove, handleContextMenu, handleScroll]);
