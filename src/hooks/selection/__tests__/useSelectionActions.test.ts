...
import type { DictEntry } from '../../../libs/selection';
...
describe('useSelectionActions - race condition prevention', () => {
    ...
    it('should only accept result from latest lookup request (sequence gating)', async () => {
        ...
        const afterFirstResolve = setState.mock.calls.filter((_call, idx) =>
            idx > setState.mock.calls.indexOf(secondResultCall!)
        );
