...
export async function fetchAppleChartIds(country: string, limit: number = 50, signal?: AbortSignal): Promise<string[]> {
    const url = `${APPLE_RSS_BASE_URL}/${country}/podcasts/top/${limit}/podcasts.json`;
...
export async function lookupPodcastsByIds(ids: string[], country: string, signal?: AbortSignal): Promise<RecommendedPodcast[]> {
    ...
    const url = `${ITUNES_LOOKUP_URL}?id=${batchIds.join(',')}&entity=podcast&country=${country}`;
    const requestKey = getRequestKey(url);
...
export async function fetchAppleTopPodcastsWithLookup(country: string, limit: number = 50, signal?: AbortSignal): Promise<RecommendedPodcast[]> {
    const ids = await fetchAppleChartIds(country, limit, signal);
