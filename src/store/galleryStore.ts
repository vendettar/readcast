...
// Abort controller management for gallery requests
...
function handleDbWriteError(operation: string, toastKey: TranslationKey, error: unknown): void {
    console.error(`[GalleryStore] Failed to ${operation}:`, error);
...
export type GalleryView = 'search' | 'subscriptions' | 'favorites' | 'podcast' | 'episode';
...
const SETTING_KEY_COUNTRY = 'gallery_country';
...
export const useGalleryStore = create<GalleryState>((set, get) => ({
    ...
    searchLoading: false,
    searchErrorKey: null,
    country: DEFAULT_COUNTRY, // Will be loaded from IDB on open
    ...
    setCountry: (country) => {
        ...
        DB.setSetting(SETTING_KEY_COUNTRY, country).catch((err) => {
            console.error('[GalleryStore] Failed to save setting:', err);
        });
    },
    ...
    performSearch: async (query) => {
        ...
        try {
            ...
            console.error('[GalleryStore] Search failed:', error);
            set({
    ...
    selectPodcast: async (podcast) => {
        ...
        try {
            ...
            console.error('[GalleryStore] Failed to load podcast feed:', error);
            set({
    ...
    loadSubscriptions: async () => {
        ...
        try {
            ...
            console.error('[GalleryStore] Failed to load subscriptions:', error);
    ...
    subscribe: async (podcast) => {
        ...
        try {
            ...
            const subs = [...get().subscriptions.filter(s => s.feedUrl !== sub.feedUrl), sub];
    ...
    addFavorite: async (podcast, episode) => {
        ...
        try {
            ...
            const favs = [...get().favorites.filter(f => f.key !== key), fav];
            set({ favorites: favs });
        } catch (error) {
