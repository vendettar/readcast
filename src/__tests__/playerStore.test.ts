...
describe('PlayerStore', () => {
    ...
    describe('Audio State', () => {
        it('should set audio URL with metadata', () => {
            const { setAudioUrl } = usePlayerStore.getState();

            setAudioUrl('http://example.com/audio.mp3', 'Test Episode', 'http://example.com/cover.jpg');

            const state = usePlayerStore.getState();
            expect(state.audioUrl).toBe('http://example.com/audio.mp3');
            expect(state.audioTitle).toBe('Test Episode');
            expect(state.coverArtUrl).toBe('http://example.com/cover.jpg');
